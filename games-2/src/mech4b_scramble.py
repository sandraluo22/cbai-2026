"""MECH4b (2026-08-03): does STRUCTURAL CONSISTENCY matter? The history format
...
RUN_DIR = os.environ.get("RUN_DIR", "runs/mech4b_scramble")
...
def main():
    ...
    for si, (roll, ts) in enumerate(streams):
        ...
        for cond, fmts in CONDS.items():
            for content in ("planted", "base"):
                ...
                json.dump({"per_cell": results}, open(os.path.join(RUN_DIR, "mech4b.json"), "w"))
        print(f"[mech4b] stream {roll} done", flush=True)
    ...
    for cond in CONDS:
        ...
        print(f"[mech4b] === {cond}: capture {p - b:.3f} (planted {p:.3f} base {b:.3f})", flush=True)
    json.dump({"per_cell": results, "summary": summary},
              open(os.path.join(RUN_DIR, "mech4b.json"), "w"), indent=1)
