...
RUN_DIR = os.environ.get("RUN_DIR", "runs/logit_lens_v3")
...
def main():
    ...
    def fids(w):
        # answer slot is prefilled with "**": tokens have no leading space; both cases
        return {tok(v, add_special_tokens=False)["input_ids"][0]
                for v in (w, w.capitalize())}
    fam_ids = torch.tensor(sorted({i for w in PLANT8 + ["plant", "plants"]
                                   for i in fids(w)})).to(dev)
        ...
        for cell in CELLS:
            ...
            with __import__("torch").no_grad():
                ...
                for l in range(1, nL + 1):
                    ...
                    # last entry is already final-normed by HF -> no second norm
