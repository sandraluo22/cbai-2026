...
Env: MODEL(QwenInst32) SRC_DIR(runs/qwen32_strict) START_FILE N_STATES(8) K(64)
     TEMP(0.7) SEED(0) RUN_DIR(runs/shuffle_transcript)
"""
...
RUN_DIR = os.environ.get("RUN_DIR", "runs/shuffle_transcript")
...
def main():
    ...
    # pick stuck states: dominant A family with >=3 members, K = min(len, 12) rounds
    states = []
    for roll, ts in sorted(games.items(), key=lambda kv: -len(kv[1])):
        ...
        if not fams:
            continue
        famp, cnt = fams.most_common(1)[0]
        if cnt < 3:
            continue
        Kr = min(len(ts), 12)
        states.append((roll, ts[:Kr], famp))
