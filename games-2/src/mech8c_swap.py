...
RUN_DIR = os.environ.get("RUN_DIR", "runs/mech8c_swap")
...
def main():
    ...
    for cx, cy in PAIRS:
        ...
        Xpool, Ypool = CATLISTS[cx][:12], CATLISTS[cy][:12]
        found = None
        for X1 in itertools.combinations(Xpool, 6):
            lx = tlen(body_of(list(X1))[0])
            for Yc in itertools.combinations(Ypool, 6):
                if tlen(body_of(list(Yc))[0]) == lx:
                    X2c = [w for w in Xpool if w not in X1][:6]
                    if tlen(body_of(X2c)[0]) == lx:
                        found = (list(X1), X2c, list(Yc))
                        break
            if found:
                break
        if not found:
