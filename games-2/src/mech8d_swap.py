"""MECH8 (2026-08-08): is B's RULE compactly encoded in A's state? Causal test.
...
RUN_DIR = os.environ.get("RUN_DIR", "runs/mech8d_swap")
...
def main():
    ...
    def body_of(bwords, sa="ledger", sb="marble"):
        hist = [(sb, sa)] + list(zip(bwords, FILLER))
        used = {sa, sb} | set(bwords) | set(FILLER)
    ...
    for cx, cy in PAIRS:
        ...
        Xpool, Ypool = CATLISTS[cx][:16], CATLISTS[cy][:16]
        # precompute lengths per subset (sampled) and bucket by length
        rng2 = __import__("random").Random(0)
        def subsets(pool, n=400):
            seen = set()
            out = []
            for _ in range(n * 3):
                c = tuple(sorted(rng2.sample(pool, 6)))
                if c not in seen:
                    seen.add(c)
                    out.append(list(c))
                if len(out) >= n:
        ...
        Ybuck = defaultdict(list)
        for Yc in Ysubs:
            Ybuck[tlen(body_of(Yc)[0])].append(Yc)
        found = None
        for X1 in Xsubs:
            lx = tlen(body_of(X1)[0])
            if not Ybuck.get(lx):
                continue
            X2cands = [c for c in Xsubs if not (set(c) & set(X1))]
            X2ok = [c for c in X2cands if tlen(body_of(c)[0]) == lx]
            if X2ok:
                found = (X1, X2ok[0], Ybuck[lx][0])
                break
        if not found:
            print(f"[m8c] {cx}->{cy}: no length match found, skipping", flush=True)
            continue
        X1, X2, Y = found
        print(f"[m8c] {cx}->{cy}: matched at {tlen(body_of(X1)[0])} tokens, no pads",
              flush=True)
        bX, uX = body_of(X1)
        bX2, _ = body_of(X2)
        bY, uY = body_of(Y)
