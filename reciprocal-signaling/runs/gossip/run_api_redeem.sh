...
A=11111111111111111111; B=00000000001111111111
for SEED in 0 1; do
  MODEL=claude-sonnet-5 VAR=curve SCHED="$A;$B" ROUNDS=20 STEPS=40 SEED=$SEED \
    OUT=duelredeem/claude-sonnet-5_s$SEED python3 qsg_api.py 2>&1 | tee -a api_redeem.log
done
echo API_REDEEM_DONE | tee -a api_redeem.log
