"""Per-token DAS firing over MANY Pile documents, for the transcript viewer — now with the multi-head
subspaces. Per token: share of head-output energy in (a) the single-head rank-16 subspace (L14H26),
...
Out: <OUTDIR>/pile_das_transcripts2<OUTTAG>_<model>.json
"""
...
def main():
    ...
    sl = {"a": slice(26 * hd, 27 * hd), "b": slice(19 * hd, 20 * hd), "e": slice(26 * hd, 27 * hd)}  # a=14:26 b=14:19 e=2:26
    ...
    for ex in ds:
        ...
        za = caps[14][0, :, sl["a"]].float(); zb = caps[14][0, :, sl["b"]].float(); ze = caps[2][0, :, sl["e"]].float()
    ...
    out = {"model": tag, "ndocs": cnt, "maxtok": MAXTOK,
           "chance": {"s": round(16 / hd, 4), "q": round(32 / (2 * hd), 4), "r": round(32 / (3 * hd), 4)},
           "docs": docs}
    pth = f"{OUTDIR}/pile_das_transcripts2{OUTTAG}_{tag}.json"
